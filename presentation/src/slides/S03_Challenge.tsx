  ...
  return (
    <LightSlide title="The Fraud Detection Challenge" num={3}>
      <div className="flex gap-5 h-full">
        ...
        <div className="flex flex-col gap-2 flex-1">
          {challenges.map((c, i) => (
            <motion.div key={i} initial={{ opacity: 0, x: -12 }} animate={{ opacity: 1, x: 0, transition: { delay: i * 0.08 } }}>
              <AccentCard accent={c.color}>
                <div className="font-semibold text-sm mb-1" style={{ color: C.textDark }}>{c.title}</div>
                <div className="text-xs leading-relaxed" style={{ color: C.textMid }}>{c.desc}</div>
              </AccentCard>
            </motion.div>
          ))}
        </div>
        ...
        {/* Right: stats */}
        <motion.div className="flex flex-col gap-2 w-36" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0, transition: { delay: 0.3 } }}>
          {/* Big fraud rate stat */}
          <div className="flex-1 flex flex-col items-center justify-center rounded text-center px-2" style={{ background: C.navyDark, border: `1.5px solid ${C.teal}` }}>
            <div className="text-4xl font-black leading-none" style={{ color: C.tealBright }}>3.5%</div>
            <div className="text-xs mt-1" style={{ color: '#94A3B8' }}>fraud rate<br />in IEEE-CIS</div>
          </div>
