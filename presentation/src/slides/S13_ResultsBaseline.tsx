...
export function S13_ResultsBaseline() {
  return (
    <LightSlide title="Results: Baseline — Full Features, Imbalanced Data" num={13}>
      <div className="flex flex-col h-full gap-2">
        ...
        <div className="flex gap-5 flex-1">
          ...
          {/* Table */}
          <motion.div className="w-72" initial={{ opacity: 0, x: 12 }} animate={{ opacity: 1, x: 0, transition: { delay: 0.2 } }}>
            <div className="rounded overflow-hidden" style={{ border: '1px solid #e2e8f0' }}>
              <div className="grid grid-cols-6 text-xs font-bold text-white py-1.5" style={{ background: C.navyDark }}>
                {['Model', 'AUC', 'PR-AUC', 'F1', 'Prec.', 'Rec.'].map(h => (
                  <div key={h} className="text-center">{h}</div>
                ))}
              </div>
              {tableRows.map(({ model, mc, auc, prauc, f1, prec, rec }, ri) => (
                <div key={model} className="grid grid-cols-6 text-xs py-2 border-t" style={{ background: ri % 2 === 0 ? C.bgCard : '#F1F7FF', borderColor: '#e2e8f0' }}>
                  <div className="flex items-center pl-1 gap-1">
                    <div className="w-1 self-stretch" style={{ background: mc }} />
                    <span className="font-bold ml-1" style={{ color: mc }}>{model}</span>
                  </div>
