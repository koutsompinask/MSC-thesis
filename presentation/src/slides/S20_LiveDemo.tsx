  ...
  return (
    <DarkSlide>
      <div className="w-full h-full flex flex-col">
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-3" style={{ background: C.navyMid, borderBottom: `2px solid ${C.teal}` }}>
          <div className="flex items-center gap-3">
            <div className="w-1.5 h-6 rounded" style={{ background: C.teal }} />
            <div>
              <div className="text-sm font-bold text-white">Live Demo</div>
              <div className="text-xs" style={{ color: C.tealBright }}>LightGBM · Fraud Probability + Explainability</div>
            </div>
          </div>
          <div className="flex items-center gap-3 text-xs" style={{ color: C.textMuted }}>
            ...
            <span>Threshold: <strong style={{ color: C.amber }}>0.0419</strong></span>
          </div>
        </div>
        ...
        <div className="flex-1 flex overflow-hidden">
          {/* Pipeline column */}
          <div className="px-5 py-4 border-r flex-shrink-0 w-44" style={{ borderColor: '#1A3A5C' }}>
            <PipelineViz />
          </div>
          ...
          <div className="flex-1 flex flex-col p-4 gap-3 overflow-hidden">
            ...
            {error && (
              <div className="text-xs p-2 rounded" style={{ background: '#1A0A0A', color: C.red, border: `1px solid ${C.red}` }}>
                {error}
              </div>
            )}
            ...
            <AnimatePresence>
              {isDone && result && (
                ...
                >
                  {/* Probability gauge */}
                  <div
                    className="flex flex-col items-center justify-center rounded-lg p-4 flex-shrink-0"
                    style={{ background: C.navyMid, border: `1px solid #1A3A5C`, minWidth: '160px' }}
                  >
            ...
            {isDone && (
              <motion.button
                className="self-end text-xs px-3 py-1.5 rounded"
